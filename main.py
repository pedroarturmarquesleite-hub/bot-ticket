...
async def enviar_log_fechamento_ticket(guild, canal, closed_by_id):
    ...
    embed = discord.Embed(
        title=f"??? — {tipo_base} {canal.name} (Automático)",
    ...
    embed.add_field(name="??? Horário", value=horario_txt, inline=False)
    embed.add_field(
        name="?? Detalhes",
        value=(
            f"Tipo: `{tipo}`\n"
...
class botd(discord.Client):
    ...
    async def _avisar_aceite_pix_brainrot(self, canal, comprador, vendedor):
        ...
        if not aceite_canal_id:
            await enviar_fluxo(
                canal,
                "?? Canal de aceite não configurado. Um administrador deve usar `/setaceite`.",
                cor=cor_paleta("erro")
            )
            return
        ...
        try:
            ...
            await enviar_fluxo(
                canal,
                "?? Sem permissão para enviar no canal de aceite configurado. "
...
class TaxaView(discord.ui.View):
    ...
    async def comprador(self, interaction, button):
        ...
        msg = (
            ...
            f"??? Vendedor receberá: R$ {self.valor:.2f}"
        )
    ...
    async def vendedor(self, interaction, button):
        ...
        msg = (
            ...
            f"??? Vendedor pagará taxa: R$ {self.taxa:.2f}\n"
            f"??? Vendedor receberá: R$ {self.valor:.2f}"
        )
...
class TradeSetupView(discord.ui.View):
    ...
    async def finalizar(self, interaction):
        ...
        if middle is None:
            ...
            await enviar_fluxo(
                self.canal,
                "?? O Middle responsável não foi encontrado, aguardando novo aceite.",
                cor=cor_paleta("erro")
            )
            return
